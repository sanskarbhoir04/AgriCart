...
$couponCodeCol = agi_pick_col($conn, 'orders', ['coupon_code']);
$discountCol  = agi_pick_col($conn, 'orders', ['product_discount', 'discount_amount', 'discount']);
$couponCol    = agi_pick_col($conn, 'orders', ['coupon_discount', 'coupon_amount']);
...
// 4. Authorization
// ---------------------------------------------------------------------
// An admin session alone isn't enough to view someone else's invoice —
// that would let e.g. a Support-only admin (no finance permissions) see
// full GSTIN/payment details, bypassing the same granular RBAC the rest
// of the admin panel enforces. Require the real 'finance.view'
// permission for the admin case; buyer/seller viewing their own order
// is unaffected.
if (agi_is_admin()) {
    require_once __DIR__ . '/includes/permissions.php';
}
$agiAdminCanViewInvoice = agi_is_admin() && function_exists('hasPermission') && hasPermission('finance.view');
if (!$isBuyer && !$viewerIsSellerHere && !$agiAdminCanViewInvoice) {
...
// Fallback: no resolvable seller_id at all — use each item's carried
// seller_name (from the su.$nameCol JOIN) instead of a blank card.
$fallbackSellerNames = [];
if (!$sellerCard && !$multiSellerCards) {
    foreach ($items as $it) {
        $sn = trim((string)($it['seller_name'] ?? ''));
        if ($sn !== '' && !in_array($sn, $fallbackSellerNames, true)) {
            $fallbackSellerNames[] = $sn;
        }
    }
}
...
if ($sellerCard && function_exists('agri_buyer_invoice_signatory_block')) {
    ...
    foreach ($fallbackSellerNames as $sn) {
        $agiSignatoryBlocks[] = [
            'for_name' => $sn, 'signature_path' => null, 'stamp_path' => null,
            'signatory_name' => null, 'designation' => null,
            'missing_signature' => true, 'missing_stamp' => true,
        ];
    }
}
...
$lineSubtotal = 0.0; $lineDiscount = 0.0; $lineGst = 0.0;
...
// CGST/SGST vs IGST — same interstate test pages/invoice.php and
// pages/seller-invoice.php already use (compare seller state to delivery
// state). This file resolves seller info via $sellerCard/$multiSellerCards
// rather than a per-item join, so for a mixed-vendor order we treat the
// whole invoice as interstate if ANY seller is outside the buyer's state
// (the safer assumption when sellers disagree, rather than guessing).
$deliveryStateForGst = trim((string)($order['delivery_state'] ?? ''));
$sellerStatesInOrder = [];
if ($sellerCard) { $sellerStatesInOrder[] = trim((string)($sellerCard['seller_state'] ?? '')); }
foreach ($multiSellerCards as $msc) { $sellerStatesInOrder[] = trim((string)($msc['seller_state'] ?? '')); }
$sellerStatesInOrder = array_filter($sellerStatesInOrder, fn($s) => $s !== '');
$isInterState = $deliveryStateForGst === '' || empty($sellerStatesInOrder);
foreach ($sellerStatesInOrder as $ss) {
    if (strcasecmp($ss, $deliveryStateForGst) !== 0) { $isInterState = true; break; }
    $isInterState = false;
}
if ($isInterState) {
    $igst = $gst; $cgst = 0.0; $sgst = 0.0;
} else {
    $cgst = round($gst / 2, 2);
...
if ($isCod) {
    ...
    $resolvedPaymentStatus = $isDelivered ? 'paid' : ($rawPaymentStatus ?: 'pending');
} else {
    $resolvedPaymentStatus = $rawPaymentStatus ?: 'paid';
}
  ...
  <div class="agi-sheet agi-print-area" id="agiInvoiceSheet">
          ...
          <tr>
            ...
            <td>
              ...
              <div class="agi-prod-sub">
                ...
                <span data-agi="soldByShort">Sold by</span>: <?php echo agi_e($it['seller_name'], 'AgriCart Seller'); ?>
              </div>
            </td>
    ...
    <div class="agi-lower">
      ...
      <div class="agi-card">
        ...
        <p><i class="fa-solid fa-envelope"></i> support@agricart.in</p>
        <p><i class="fa-solid fa-globe"></i> www.agricart.in</p>
