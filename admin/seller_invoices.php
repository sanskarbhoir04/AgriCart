<?php
// =====================================================================
// admin/seller_invoices.php — Admin-side view of every Seller Invoice
// generated across the marketplace (admin/seller_invoices.php).
// Read-only: admins can search, filter, open, and print/download any
// seller's invoice via seller/invoice.php?order_id=X&seller_id=Y (that
// page already allows an admin session to view any seller's invoice —
// see sinv_is_admin() in seller/invoice.php).
// =====================================================================
...
// Settlement status isn't a stored column (see agri_seller_invoice_settlement_status()
// in includes/seller_functions.php — it's always derived live so it can
// never show "Settled"/"Refunded" before that's actually true), so we
// filter on it in PHP after fetching this page's base rows.
$baseSql = "SELECT si.id, si.invoice_number, si.seller_id, si.order_id, si.gross_amount,
                   ...
                   spp.signature_status, spp.stamp_status
            $joinSql
            $whereSql
...
$summaryGross = 0.0; $summaryCharges = 0.0; $summaryNet = 0.0;
$filteredRows = [];
foreach ($allRows as $r) {
    $r['settlement_status'] = agri_seller_invoice_settlement_status($conn, (int)$r['seller_id'], (int)$r['order_id']);
...
$pageTitle = 'Seller Invoices';
$pageSubtitle = 'Invoices generated for seller commissions and platform charges.';
$activeTeamTab = 'seller_invoices';
include __DIR__ . '/includes/team_layout_top.php';
?>
<div class="card">
    <div class="card-head">
        <h2>Seller Invoices <span style="color:var(--muted);font-weight:400">(<?php echo $total; ?>)</span></h2>
    </div>
        ...
        <table class="data-table" style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="text-align:left;border-bottom:1px solid #eee;">
                    ...
                    <th style="padding:10px;">Signature/Stamp</th>
                    <th style="padding:10px 20px;">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!$rows): ?>
                    <tr><td colspan="10" style="padding:24px 20px;color:var(--muted);">No seller invoices match these filters.</td></tr>
                <?php endif; ?>
                <?php foreach ($rows as $r): ?>
                    <tr style="border-bottom:1px solid #f2f2f2;">
                        ...
                        <td style="padding:10px;">
                            <?php $sigOk = ($r['signature_status'] ?? 'missing') === 'uploaded'; $stampOk = ($r['stamp_status'] ?? 'missing') === 'uploaded'; ?>
                            <span class="tag <?php echo $sigOk ? 'active' : 'pending'; ?>" style="margin-right:4px;">Sig <?php echo $sigOk ? '✓' : '✗'; ?></span>
                            <span class="tag <?php echo $stampOk ? 'active' : 'pending'; ?>">Stamp <?php echo $stampOk ? '✓' : '✗'; ?></span>
                        </td>
                        <td style="padding:10px 20px;white-space:nowrap;">
                            <div class="action-menu-wrap">
                                ...
                                <div class="action-menu">
                                    <a target="_blank" rel="noopener" href="../seller/invoice.php?order_id=<?php echo (int)$r['order_id']; ?>&seller_id=<?php echo (int)$r['seller_id']; ?>"><i class="fa-solid fa-eye"></i> View Invoice</a>
                                </div>
                            </div>
                        </td>
                    </tr>
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination" style="padding:14px 20px;">
            <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                <a href="?q=<?php echo urlencode($search); ?>&settlement=<?php echo urlencode($settlementFilter); ?>&from=<?php echo urlencode($dateFrom); ?>&to=<?php echo urlencode($dateTo); ?>&page=<?php echo $p; ?>" class="<?php echo $p === $page ? 'active' : ''; ?>"><?php echo $p; ?></a>
            <?php endfor; ?>
        </div>
    <?php endif; ?>
</div>
