...
// Matches the "Pending" count on the Invoices Overview card (payment_status
// = 'pending' exactly) — kept in sync so the card and the list it links to
// never disagree.
if ($payStatusFilter !== '' && $payStatusCol) { $where[] = "o.$payStatusCol = ?"; $types .= 's'; $params[] = $payStatusFilter; }
if ($orderStatusFilter !== '' && $oStatusCol) { $where[] = "o.$oStatusCol = ?"; $types .= 's'; $params[] = $orderStatusFilter; }
...
<form method="get" class="filters" style="padding:14px 20px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
    ...
    <select name="order_status">
        ...
        <?php foreach (['placed' => 'Placed', 'packed' => 'Packed', 'shipped' => 'Shipped', 'delivered' => 'Delivered', 'cancelled' => 'Cancelled'] as $val => $label): ?>
            <option value="<?php echo $val; ?>" <?php echo $orderStatusFilter === $val ? 'selected' : ''; ?>><?php echo htmlspecialchars($label); ?></option>
        <?php endforeach; ?>
    </select>
    <?php endif; ?>
        ...
        <tbody>
            ...
            <?php foreach ($rows as $r): ?>
                <tr style="border-bottom:1px solid #f2f2f2;">
                    ...
                    <td style="padding:10px 20px;white-space:nowrap;text-align:center;">
                        <button type="button" class="row-kebab" data-view="invoice.php?order_id=<?php echo (int)$r['order_id']; ?>" data-order="index.php?tab=orders&order_id=<?php echo (int)$r['order_id']; ?>" aria-label="Actions"><i class="fa-solid fa-ellipsis-vertical"></i></button>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<?php if ($totalPages > 1): ?>
    <div class="pagination" style="padding:14px 20px;">
        <?php for ($p = 1; $p <= $totalPages; $p++): ?>
            <a href="?type=buyer&q=<?php echo urlencode($search); ?>&pay_status=<?php echo urlencode($payStatusFilter); ?>&order_status=<?php echo urlencode($orderStatusFilter); ?>&from=<?php echo urlencode($dateFrom); ?>&to=<?php echo urlencode($dateTo); ?>&page=<?php echo $p; ?>" class="<?php echo $p === $page ? 'active' : ''; ?>"><?php echo $p; ?></a>
        <?php endfor; ?>
    </div>
<?php endif; ?>

<style>
.row-kebab{width:32px;height:32px;border-radius:8px;border:1px solid var(--border);background:#fff;color:var(--text);font-size:15px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:background .15s ease,border-color .15s ease}
.row-kebab:hover,.row-kebab.open{background:var(--bg-soft);border-color:var(--primary);color:var(--primary)}
.row-action-menu{position:fixed;min-width:210px;background:#fff;border:1px solid var(--border);border-radius:10px;box-shadow:0 10px 28px rgba(0,0,0,.14);padding:6px;z-index:999;display:none}
.row-action-menu.open{display:block}
.row-action-menu a{display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:7px;font-size:13px;font-weight:600;color:var(--text);text-decoration:none}
.row-action-menu a:hover{background:var(--bg-soft);color:var(--primary)}
.row-action-menu a i{width:16px;text-align:center;color:var(--muted)}
</style>
<div id="rowActionMenu" class="row-action-menu">
    <a id="rowActionView" target="_blank" rel="noopener"><i class="fa-solid fa-eye"></i> View / Print / Download</a>
    <a id="rowActionOrder"><i class="fa-solid fa-truck-fast"></i> Order</a>
</div>
<script>
(function(){
    var menu = document.getElementById('rowActionMenu');
    var viewLink = document.getElementById('rowActionView');
    var orderLink = document.getElementById('rowActionOrder');
    var openBtn = null;
    ...
    document.querySelectorAll('.row-kebab').forEach(function(btn){
        btn.addEventListener('click', function(e){
            ...
            viewLink.href = btn.getAttribute('data-view');
            orderLink.href = btn.getAttribute('data-order');
            var r = btn.getBoundingClientRect();
            menu.style.display = 'block';
            var menuW = menu.offsetWidth;
            var left = Math.min(r.right - menuW, window.innerWidth - menuW - 8);
            var top = r.bottom + 6;
            if (top + 90 > window.innerHeight) { top = r.top - 6 - 90; }
